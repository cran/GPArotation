...
sortFac <- function(x){  # Based on Fungible faSort 
  ...
  set.seed(100)
 tst <- sortFac(Varimax(L, normalize=FALSE, randomStarts = 100))$loadings
 if( 0.001 < max(abs(varimax(L, normalize=FALSE)$loadings - tst))){
    cat("Calculated difference exceeds tolerance\n")
    cat("difference:\n")
    print(varimax(L, normalize=FALSE)$loadings - tst, digits=18)
    all.ok <- FALSE  
    } 

  set.seed(100)
 tst <- sortFac(Varimax(L, normalize=TRUE, randomStarts = 100))$loadings
 if( 0.01 < max(abs(varimax(L, normalize=TRUE)$loadings - tst))) {
    cat("Calculated difference exceeds tolerance\n")
    cat("difference:\n")
    print(varimax(L, normalize=TRUE)$loadings - tst, digits=18)
    all.ok <- FALSE  
    } 

 set.seed(99)
  v <- sortFac(oblimin(L, eps=1e-8, randomStarts = 100))$loadings 
  tst <- t(matrix(c(
  ...
  set.seed(98)
  v <- sortFac(quartimin(L, eps=1e-8, randomStarts = 100))$loadings
  tst <- t(matrix(c(
           0.3863615904740822504,  0.4745127741495974161,
          -0.0110059418769087539,  0.6458720769633764514,
          -0.0262926272350604423,  0.8961141105684561348,
...
#  # This fails with the old Random.Start
#  set.seed(97)
#  v <- sortFac(targetT(L, Target=matrix(c(rep(1,3),rep(0,6),rep(1,3)), 6,2),
#               eps=1e-5, randomStarts = 100))$loadings 
#  tst <- t(matrix(c(
...
#  # This fails with the old Random.Start
# Random starts get to a lower f value (1.8) which is a mismatch to the tst matrix 
# generated without random starts. Its f value is 5.6
# removed the test 
#  set.seed(96)
#  v <- sortFac(targetQ(L, Target=matrix(c(rep(1,3),rep(0,6),rep(1,3)), 6,2),
#               eps=1e-5, randomStarts = 100))$loadings  
#  tst <- t(matrix(c(
...
#    } 
  ...
  v <- sortFac(entropy(L, maxit=3000, eps=1e-5, randomStarts = 100))$loadings  
  tst <- t(matrix(c(
  ...
  fuzz <- 1e-4  # 4th decimal differences
  set.seed(110)
  v <- sortFac(quartimax(L, eps=1e-7, randomStarts = 100))$loadings
  tst <- t(matrix(c(
  ...
  if( fuzz < max(abs(v - tst))) {
   ...
   set.seed(110)
  v <- sortFac(GPFRSorth(L, eps = 1e-7, method = "quartimax", randomStarts = 100))$loadings
  if( fuzz < max(abs(v - tst))) {
    cat("Calculated value is not the same as test value in test rotations 9-GPFRSorth. Value:\n")
    print(v, digits=18)
    cat("difference:\n")
    print(v - tst, digits=18)
    all.ok <- FALSE  
 ...
 set.seed(90)
  v <- sortFac(Varimax(L, eps=1e-8, randomStarts = 100))$loadings  
  tst <- t(matrix(c(
  ...
  if( fuzz < max(abs(v - tst))) {
    ...
    set.seed(90)
  v <- sortFac(GPFRSorth(L, eps = 1e-7, method = "varimax", randomStarts = 100))$loadings
  if( fuzz < max(abs(v - tst))) {
    cat("Calculated value is not the same as test value in test rotations 10-GPFRSorth. Value:\n")
    print(v, digits=18)
    cat("difference:\n")
    print(v - tst, digits=18)
    all.ok <- FALSE  
  ...
  set.seed(89)
  v <- sortFac(simplimax(L, eps=1e-5, randomStarts = 100))$loadings
  tst <- t(matrix(c(
  ...
  if( fuzz < max(abs(v - tst[,2:1]))) {
    ...
    set.seed(89)
  v <- sortFac(GPFRSoblq(L, eps = 1e-5, method = "simplimax", randomStarts = 100))$loadings
  if( fuzz < max(abs(v - tst[,2:1]))) {
    cat("Calculated value is not the same as test value in test rotations 11-GPFRSoblq. Value:\n")
    print(v, digits=18)
    cat("difference:\n")
    print(v - tst, digits=18)
    all.ok <- FALSE  
  ...
  set.seed(88)
  v <- sortFac(bentlerT(L, eps=1e-8, randomStarts = 100))$loadings 
  tst <- t(matrix(c(
  ...
  set.seed(88)
   v <- sortFac(GPFRSorth(L, eps = 1e-7, method = "bentler", randomStarts = 100))$loadings
  if( fuzz < max(abs(v - tst))) {
    cat("Calculated value is not the same as test value in test rotations 12-GPFRSorth. Value:\n")
    print(v, digits=18)
    cat("difference:\n")
    print(v - tst, digits=18)
    all.ok <- FALSE  
  ...
  fuzz <- 1e-5
  set.seed(85)
  v <- sortFac(tandemI(L, eps=1e-6, maxit = 1000, randomStarts = 100))$loadings  
  tst <- t(matrix(c(
  ...
  fuzz <- 1e-5
  set.seed(84)
  v <- sortFac(tandemII(L, eps=1e-5, randomStarts = 100))$loadings 
  tst <- t(matrix(c(
  ...
  fuzz <- 1e-6
  set.seed(81)
  v <- sortFac(geominT(L, eps=1e-5, randomStarts = 100))$loadings  
  tst <- t(matrix(c(
  ...
  fuzz <- 1e-6
  set.seed(80)
  v <- sortFac(geominQ(L, eps=1e-5, randomStarts = 100))$loadings  
  tst <- t(matrix(c(
  ...
  set.seed(70)
  v <- sortFac(cfT(L, eps=1e-8, randomStarts = 100))$loadings	
  tst <- t(matrix(c(
  ...
  set.seed(60)
  v <- sortFac(cfQ(L, eps=1e-8, randomStarts = 100))$loadings	
  tst <- t(matrix(c(
           0.3863615904740822504,  0.4745127741495974161,
          -0.0110059418769087539,  0.6458720769633764514,
          -0.0262926272350604423,  0.8961141105684561348,
  ...
  fuzz <- 1e-5
  set.seed(55)
  v <- sortFac(infomaxT(L, eps=1e-5, randomStarts = 100))$loadings 
  tst <- t(matrix(c(
  ...
  fuzz <- 1e-5
  set.seed(50)
  v <- sortFac(infomaxQ(L, eps=1e-5, randomStarts = 100))$loadings 
  tst <- t(matrix(c(
 ...
 set.seed(40)
  ...
  if( fuzz < max(abs(v - tst))) {
   ...
   set.seed(35)
   v <- sortFac(oblimax(Harman8, eps=1e-5, randomStarts = 100))$loadings
  tst <- t(matrix(c(
